" URL: http://vim.wikia.com/wiki/Example_vimrc Authors:
" http://vim.wikia.com/wiki/Vim_on_Freenode Description: A minimal, but
" feature rich, example .vimrc. If you are a newbie, basing your first
" .vimrc on this file is a good choice.  If you're a more advanced user,
" building your own .vimrc based on this file is still a good idea.
" Modified: Nachikethas A. J.
...
" option).  Alternatives include using tabs or split windows instead of
" re-using the same window as mentioned above, and/or either of the
" following options: set confirm set autowriteall
...
" Usability options {{{1
"
" These are options that users frequently set in their .vimrc. Some of
" them change Vim's behaviour in ways which deviate from the true Vi
" way, but which are considered to add usability. Which, if any, of
...
" Indentation settings for using 4 spaces instead of tabs.  Do not
" change 'tabstop' from its default value of 8 with this setup.
set shiftwidth=4
set softtabstop=4
set expandtab
...
" vim-airline Enable powerline fonts for pretty Airline.  Remember to
" install one of the powerline fonts from URL:
" https://github.com/powerline/fonts prior to using the option below.
let g:airline_powerline_fonts=1

"Disable spell detection (it's always on)
let g:airline_detect_spell=0

" Don't display the spelling language
...
" Fix spelling errors not highlighted in terminal vim
...
" YouCompleteMe
let g:ycm_global_ycm_extra_conf = "~/.vim/plugs/ycm_extra_conf.py"
...
" Custom commands {{{1
"
" Delete buffer without removing the split
command Bd bp\|bd \#
